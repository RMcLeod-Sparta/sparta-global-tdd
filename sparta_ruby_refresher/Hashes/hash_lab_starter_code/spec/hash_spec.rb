...
describe 'Create tests for a hash' do
  ...
  it 'each fruit should have 4 keys' do
    expect(small_fruit_nutrition_details[:apple].length).to eq 4
    expect(small_fruit_nutrition_details[:avocado].length).to eq 4
    expect(small_fruit_nutrition_details[:kiwifruit].length).to eq 4
    expect(small_fruit_nutrition_details[:plums].length).to eq 4
  end
